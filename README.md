...
locate character in string while verify with the characters table.
...
- `len`: length of `s`.
- `c`: lookup target character.
- `accept256`: the 256 byte's acceptable characters table.
...
set the EILSEQ into the external variable errno if failed to The function `strchr_brk()` and `strchr_brkrep()`.
